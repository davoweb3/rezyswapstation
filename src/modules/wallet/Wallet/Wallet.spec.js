...
import { TEST_ACCOUNT_ADDRESS, TEST_TRANSACTION_HASH, TestWeb3Wallet } from '@/plugins/web3-wallets/test-helpers.js';
...
async function createWallet() {
    store = useTestStore();
    wallet = new Wallet({ store, api, defaulChainId: DEFAULT_CHAIN_ID });
...
describe('Wallet', () => {
    ...
    describe('setting a web3 wallet', () => {
        ...
        it.skip('should throw an error if wallet does not implement Web3Wallet interface', async () => {
            ...
            await expect(
                wallet.setWeb3Wallet(new TestWeb3Wallet2({ name: `${TEST_WALLET_NAME}2` }))
    ...
    describe('preparing of transaction object', () => {
        ...
        it('should fill `from` property by current wallet address if it is not given', async () => {
            await delay();
            expect(await wallet.prepareTransaction({})).toHaveProperty('from', TEST_ACCOUNT_ADDRESS);
        });
    });
    ...
    describe('transaction signing', () => {
        ...
        it('should set ref txStatus object with "error" status and error and emit an error event if something goes wrong', async () => {
            ...
            await wallet.signTransaction({ __errorMessage__: error, transaction: {}, txStatus });
    ...
    describe('transaction sending', () => {
        ...
        it('should set ref txStatus object with "error" status and error and emit an error event if something goes wrong', async () => {
            ...
            await wallet.sendTransaction({ __errorMessage__: error, transaction: {}, txStatus });
    ...
    describe('getTransactionFee()', () => {
        ...
        it('should calculate transaction fee', () => {
            expect(wallet.getTransactionFee('0x3B9ACA00', '0x5208')).toEqual(toBigNumber('21000000000000'));
        });
    });
});
