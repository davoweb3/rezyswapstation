...
const { enablePopoverHiding, disablePopoverHiding } = useFPopover(WALLET_BUTTON_POPOVER_ID);
let currentAction = '';
...
<template>
    <FButton class="logoutbutton fbutton-withicon" tertiary size="large" v-bind="$attrs" @click="onButtonClick">
        <AppIconset icon="close" /> {{ $t('wallet.logout') }}
    </FButton>
    <LogoutConfirmationWindow ref="window" @button-action="onButtonAction" @window-hide="onWindowHide" />
</template>
