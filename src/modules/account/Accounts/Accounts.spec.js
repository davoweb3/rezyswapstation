...
import { TEST_ACCOUNT_ADDRESS, TEST_ACCOUNT_ADDRESS2, TestWeb3Wallet } from '@/plugins/web3-wallets/test-helpers.js';
...
function createAccounts({ callback, oneAccountMode = false } = {}) {
    Accounts.registerWeb3Wallet({ name: TEST_WALLET_NAME, clas: TestWeb3Wallet });
    Accounts.registerWeb3Wallet({ name: TEST_WALLET_NAME2, clas: TestWeb3Wallet });
    store = useTestStore();
...
describe('Accounts', () => {
    ...
    describe('adding new account', () => {
        it('should add new account', async () => {
            expect(await accounts.addAccount(TEST_ACCOUNT)).toBe(true);
            expect(accounts.getAccountByAddress(TEST_ACCOUNT_ADDRESS)).toEqual(TEST_ACCOUNT);
        });
        ...
        it('should return false if account already exists', async () => {
            ...
            expect(await accounts.addAccount(TEST_ACCOUNT)).toBe(false);
        });
    ...
    describe('removing account by address', () => {
        it('should remove account by address', async () => {
            ...
            await accounts.removeAccountByAddress(TEST_ACCOUNT.address);
        ...
        it('should reset activeAccountAddress if account to be removed is active account', async () => {
            accounts.addAccount(TEST_ACCOUNT);
            await accounts.setActiveAccount(TEST_ACCOUNT);
    ...
    describe('setting active account', () => {
        it('should throw an error if there is no registered web3wallet with given name', async () => {
            await expect(accounts.setActiveAccount({ ...TEST_ACCOUNT, walletName: 'foo' })).rejects.toBeInstanceOf(
                Error
            );
        });
        ...
        it('should save active account address in the store', async () => {
            ...
            expect(accounts.store.activeAccountAddress).toBe(TEST_ACCOUNT.address);
        });
    });

    describe('setting active account by address', () => {
        it('should throw an error if account is not found', async () => {
            await expect(accounts.setActiveAccountByAddress(TEST_ACCOUNT_ADDRESS)).rejects.toBeInstanceOf(Error);
        });
