...
export class Accounts {
     ...
     */
    constructor({ store = {}, wallet = null, notifications = null, oneAccountMode = false }) {
        this.setStore(store);
    ...
    getAccountByAddress(address = '') {
        return this.#store.accounts.find((account) => addressesMatch(account.address, address));
    }
    ...
    async addAccount(account) {
        ...
        if (Accounts.isAccountOk(account)) {
            if (!this.getAccountByAddress(account.address) && Accounts.isKeystoreFileOk(account)) {
                if (this.#oneAccountMode) {
                    if (store.accounts.length > 0) {
                        await this.removeAccountByAddress(store.accounts[0].address, true);
                    }
                }
            ...
            throw new Error(`Given account should be in the form: ${JSON.stringify(Accounts.accountDef)}`);
        }
    ...
    async removeAccountByAddress(address, noNotification = false) {
        const store = this.#store;
        const index = store.accounts.findIndex((account) => addressesMatch(account.address, address));
    ...
    async removeAllAccounts() {
        if (this.#wallet.isSet()) {
            await this.#removeActiveWallet({ address: this.#wallet.address, resetWalletStore: true });
        }
    ...
    async #createWeb3WalletInstance(account) {
        ...
        if (Accounts.isWeb3WalletRegistered(walletName)) {
            ...
            return new web3WalletClass({ name: walletName, address: account.address, options: web3Wallet.options });
        } else {
    ...
    static isKeystoreFileOk(account) {
        if (account.keystoreFile && !addressesMatch(account.keystoreFile.address, account.address)) {
